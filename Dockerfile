...
FROM --platform=$BUILDPLATFORM ubuntu:20.04-slim AS build-stage
...
# Determine architecture and download Go version
...
# Install other dependencies in one layer
ENV LIBDNET_VERSION=1.14 FLEX_VERSION=2.6.4 HWLOC_VERSION=2.5.0 PCRE_VERSION=8.45 ZLIB_VERSION=1.2.13 LUAJIT_VERSION=2.1.0-beta3
RUN cd /work && \
    ...
    ./configure && make && make install && cd /work && \
    wget https://github.com/westes/flex/files/981163/flex-${FLEX_VERSION}.tar.gz && \
    tar -xvf flex-${FLEX_VERSION}.tar.gz && \
    cd flex-${FLEX_VERSION} && ./configure && make && make install && \
    cd /work && wget https://download.open-mpi.org/release/hwloc/v2.5/hwloc-${HWLOC_VERSION}.tar.gz && \
    tar -xvf hwloc-${HWLOC_VERSION}.tar.gz && \
    cd hwloc-${HWLOC_VERSION} && ./configure && make && make install && \
    cd /work && wget https://sourceforge.net/projects/pcre/files/pcre/${PCRE_VERSION}/pcre-${PCRE_VERSION}.tar.gz && \
    tar -xvf pcre-${PCRE_VERSION}.tar.gz && \
    cd pcre-${PCRE_VERSION} && ./configure && make && make install && \
    cd /work && wget https://github.com/madler/zlib/releases/download/v${ZLIB_VERSION}/zlib-${ZLIB_VERSION}.tar.gz && \
    tar -xvf zlib-${ZLIB_VERSION}.tar.gz && cd zlib-${ZLIB_VERSION} && ./configure && make && make install
